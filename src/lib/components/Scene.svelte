...
<script>
  ...
  */
  ...
  function addPostProcessing () {
    ...
    if (w > 640) {
      innerGeometry.position.x = -0.5 + (Math.sin(($coords.x / window.innerWidth)) - 0.5) * 0.05
    } else {
  ...
  function createBackground () {
    ...
    innerUniforms = {
      ...
      u_RefractionRatio: { type: 'f', value: .02 },
      u_FresnelBias: { type: 'f', value: 0.5 },
      u_FresnelScale: { type: 'f', value: .1 },
  ...
  function createInnerGeometry () {
    ...
    cubeMaterial = track(new RefractionReflectionMaterial({ uniforms: innerUniforms }))
